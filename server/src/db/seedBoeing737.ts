...
// ---------------------------------------------------------------------------
// Spirit AeroSystems (Wichita) — 737 Fuselage Integration
// Value Stream: Parts Missing & Omitted (PM&O) in the 737 Integration line
//
// Spirit AeroSystems builds major 737 fuselage sections in Wichita (formerly
// Boeing Wichita, acquired 2005): Section 41 (forward fuselage / nose) and
// Section 43 (forward cabin), joined in Wichita before shipment to Boeing
// Renton for final assembly.
//
// Systems referenced:
//   SAP ERP   — confirmed Spirit production system (materials, PP, QM modules)
//   Solumina  — iBASEt MES; confirmed Spirit system for electronic traveler,
//               work-order routing, and inspection records
//   Spirit PDM — engineering data / drawing control (generic reference)
//   OPR tracking log — shared Excel maintained by Production Control (notional
//                       but representative of common Spirit practice)
//
// Process logic and metrics are based on publicly reported 737 quality events
// and standard aerospace large-structure assembly practice; specific numbers
// are notional estimates calibrated to the scale of the program.
// ---------------------------------------------------------------------------
...
const STEPS: StepDef[] = [
  // ---- normal integration flow (Steps 1–10) --------------------------------
  {
    ...
    entry: "Engineering-released drawing revision and BOM available in Spirit PDM; MSN (Manufacturing Serial Number) assigned to this fuselage section",
    action: "Manufacturing Engineer reviews traveler package in Solumina for completeness and ECN reconciliation; Production Control opens the SAP PP production order; Solumina creates the MSN-linked traveler instance for this section",
    exit: "Released Solumina traveler for this MSN; SAP production order open and released to the floor",
    cycle: 0.5, wait: 2.0, pca: 72,
    pain: "Engineering Change Notices (ECNs) frequently arrive after the traveler is already released — sometimes mid-build — requiring traveler amendments. Spirit tracks the build-to vs. as-designed delta manually, and the reconciliation creates coordination overhead that delays build start. Traveler packages released with open TBD items are common, particularly on recently revised sections.",
    sys: "Solumina (iBASEt), SAP PP, Spirit PDM",
    dbs: "SAP ERP, Solumina MES, Spirit PDM",
    tbls: "SAP: AUFK (production order), MAST (BOM header), STPO (BOM items); Solumina: traveler/router instance",
    etl: "PDM drawing release → Solumina traveler update (manual trigger by Manufacturing Engineer per ECN)",
  },
  {
    n: 2, name: "BOM-to-Kit (Kitting)",
    entry: "Released SAP production order; component requirements list (RESB) available; Solumina kit list printed or queued for MCC",
    ...
    sys: "SAP MM (inventory management), Solumina (kit list generation), bonded stores management",
    dbs: "SAP ERP, Solumina MES",
    tbls: "SAP: RESB (component requirements), MARD (storage location stock), MARC (plant/material), MCHB (batch stock)",
    etl: "SAP RESB → Solumina kit list (interface job; triggered per production order release)",
  },
  {
  ...
  },
  {
    ...
    action: "MCC or material handler moves kit containers from bonded stores to the assigned station on the integration floor; places containers in the station staging rack in the designated bay location; logs delivery in Solumina traveler",
    exit: "Kit at station; Solumina traveler updated with delivery timestamp",
    cycle: 0.25, wait: 0.5, pca: 88,
    pain: "Solumina delivery-log update is manual and frequently delayed or missed — the floor is active and the MCC is managing multiple simultaneous deliveries. Occasionally kits are staged at the wrong station (adjacent fuselage sections look similar in the integration bay). Station congestion during high-build-rate periods leads to kits being stacked, which obscures whether the correct kit is present at the right station.",
    sys: "Solumina (iBASEt), production floor staging racks",
    dbs: "Solumina MES",
  },
  {
    n: 5, name: "Station Authorization & Tooling Verification",
    entry: "Kit at station; Solumina traveler open; required special tooling list identified in traveler",
    action: "Lead Mechanic activates the traveler step in Solumina; verifies required special tools and fixtures (jigs, calibrated torque multipliers, drill templates) are checked out from tool crib and current on calibration; briefs Technicians on the operation sequence and any active ECNs or traveler amendments affecting this station",
    exit: "Traveler step activated in Solumina; all required tooling at station with calibration confirmed; team briefed",
    cycle: 0.25, wait: 0.5, pca: 82,
    pain: "Calibrated special tooling is shared across concurrent sections and may be in use elsewhere or out of calibration when needed, delaying start. The Solumina traveler may reference tooling from a superseded engineering revision if the ME did not complete the traveler amendment in time. This step is also the first moment a Technician physically opens the kit — and the most common point of PM&O discovery.",
    sys: "Solumina (iBASEt), Tool Crib management system",
    dbs: "Solumina MES, Tool Crib system",
  },
  {
    ...
    sys: "Solumina (iBASEt), Spirit PDM (drawing references), calibrated torque tooling",
    dbs: "Solumina MES, Spirit PDM",
    tbls: "Solumina: traveler step records, fastener lot log (manual entry)",
  },
  {
    ...
    exit: "All rough-in systems routed, supported, and protected; no FOD; visual inspection by Lead Mechanic; Solumina rough-in steps signed off",
    ...
    sys: "Solumina (iBASEt), Spirit PDM (routing and stress drawings), SAP MM (bulk hardware stock)",
    dbs: "Solumina MES, SAP ERP, Spirit PDM",
  },
  {
    ...
    exit: "All traveler inspection points DI-signed in Solumina; SAP QN opened for any findings; section cleared to proceed to systems test",
    ...
    sys: "Solumina (iBASEt), SAP QM (Quality Notifications — QM01)",
    dbs: "SAP ERP (QM module), Solumina MES",
  ...
  },
  {
    ...
    action: "Systems technicians perform electrical continuity and insulation resistance checks on wiring harnesses through the join zone; hydraulic lines are pressure-tested per the test specification; pneumatic ducts are leak-checked; test results are recorded in Solumina",
    exit: "Test results recorded in Solumina; pass/fail for each system segment documented; any failures written up for troubleshooting disposition",
    cycle: 2.0, wait: 1.0, pca: 74,
    pain: "Test failures that trace to mis-routed harnesses or missing connectors (a PM&O from Step 7) require troubleshooting that can consume 0.5–2 additional days. Test records are entered manually in Solumina but traceability to the specific harness part number and lot is not always captured, making root-cause analysis difficult. Pressure test equipment is shared across stations and occasionally unavailable when needed.",
    sys: "Solumina (iBASEt), portable electrical and hydraulic test equipment",
    dbs: "Solumina MES",
  },
  {
    n: 10, name: "Section Buy-Off & Shipment Authorization",
    entry: "All Solumina traveler steps signed off; all open OPRs closed or formally deferred with QE-approved written disposition; Quality Engineer scheduled for review",
    action: "Quality Engineer reviews the complete traveler package for step completeness, inspection sign-offs, and OPR closure status; confirms no open SAP Quality Notifications without written dispositions; issues the Section Conformance sign-off document; Production Control coordinates with Boeing Renton schedule for the shipment window; section transferred to shipping preparation",
    exit: "Signed Section Conformance document; SAP production order technically completed (TECO); Solumina traveler closed; section on shipment schedule to Boeing Renton",
    cycle: 0.5, wait: 1.0, pca: 82,
    pain: "Open OPRs at buy-off are the single biggest driver of section schedule slip. When the OPR backlog accumulates — particularly near program delivery milestones — weekend overtime and emergency expedites become the norm and the VP of Supply Chain & Fabrication is frequently drawn into disposition decisions. The buy-off review requires manually reconciling three data sources (Solumina traveler, SAP QN status, and the OPR tracking Excel) because these systems are not integrated.",
    sys: "Solumina (iBASEt), SAP PP (production order close — TECO), OPR tracking log (shared Excel)",
    dbs: "SAP ERP, Solumina MES, OPR tracking log",
  ...
  },
  {
    ...
    action: "Quality Engineer and Manufacturing Engineer jointly review the impact: Can work continue at other traveler steps while the part is expedited (parallel path)? Is there a drawing-approved substitution? Does the missing part affect structural integrity or airworthiness, requiring Boeing Customer engineering review? Written disposition is documented in the SAP Quality Notification activity record.",
    ...
    sys: "SAP QM (QN activity update), Spirit PDM (drawing review for substitution or disposition), Solumina (downstream traveler dependency check)",
    dbs: "SAP ERP, Spirit PDM, Solumina MES",
    tbls: "SAP: QMMA (QN activities), QMEL (QN status); Solumina: traveler dependency map (manual lookup)",
  },
  {
    ...
    pain: "Single-source proprietary parts — Spirit-designed brackets, doublers, composite clips, and window-frame details — have no approved alternate source. When these are the missing part, the section may sit at station for 10–21 days waiting for the supplier to manufacture and ship. Spirit has limited real-time visibility into supplier subcomponent lead times, so expedite date-promises are frequently optimistic. The gap between the supplier's first-promised delivery date and actual receipt has averaged several days over recent build cycles.",
  ...
  },
  {
    ...
    entry: "Replacement part received at Spirit and cleared through receiving inspection; engineering disposition authorizing out-of-sequence (OOS) installation; Lead Mechanic and QE notified",
    action: "Technician installs the part out of its planned sequence, which may require partial disassembly (removing access panels, relocating harnesses, or removing previously installed hardware to create access), use of elevated work platforms and extended-reach tooling, and re-installation of displaced components; Lead Mechanic witnesses the installation; steps documented in the SAP QN and retroactively completed in Solumina",
    exit: "Part installed per drawing; Lead Mechanic installation sign-off; Solumina traveler step retroactively completed; SAP QN updated with installation record; ready for OPR closure inspection (Step 16)",
    ...
    sys: "Solumina (iBASEt — retroactive traveler step completion), SAP QM (QN activity update), access platform / work stand management",
    dbs: "Solumina MES, SAP ERP",
    tbls: "SAP: QMMA (QN activities); Solumina: traveler retroactive completion record",
  },
  {
    ...
    action: "Designated Inspector physically inspects the installation: confirms correct part number, correct drawing revision, correct orientation, fastener torque (if applicable), sealant application (if applicable), and no FOD left in the work zone; DI signs the SAP QN closure record; Quality Engineer performs the formal QN closure transaction in SAP QM (QM02); Solumina traveler updated to reflect OPR resolution; OPR removed from open backlog in the tracking log",
    exit: "SAP QN formally closed (status 'Completed') with DI and QE signatures; Solumina traveler reflects step completion; OPR tracking log updated; Production Control notified that OPR is cleared",
    cycle: 0.5, wait: 1.5, pca: 72,
    pain: "OPR closure inspections compete directly for DI time with normal build-progress traveler sign-offs on active sections. During high-OPR periods, the DI is context-switching between closure inspections and normal traveler sign-offs across multiple sections simultaneously, creating task-switching overhead and extending total DI calendar time per section. Not all DIs are trained on the SAP QM02 closure transaction, requiring QE intervention to complete the SAP record — adding a hand-off delay. Retroactive Solumina traveler updates are sometimes missed, leaving the traveler in an incomplete state that re-flags at section buy-off review.",
    sys: "SAP QM (QN closure — QM02 transaction), Solumina (iBASEt — retroactive traveler update), OPR tracking log",
    dbs: "SAP ERP (QM module), Solumina MES, OPR tracking log",
...
const PERSONAS: PersonaDef[] = [
  { n: 1,  name: "Assembly Technician",           role: "Journey Mechanic / Mechanic A",       fn: "Integration / Assembly",      scope: "local",  resp: "Performs structural integration and systems rough-in tasks per the Solumina traveler; installs fasteners, routes harnesses, and reports parts shortages to Lead Mechanic.",         auth: "Authorized to perform traveler-assigned steps only. Cannot deviate from drawing or substitute parts without a written engineering disposition." },
  { n: 2,  name: "Lead Mechanic",                 role: "Lead Mechanic",                       fn: "Integration / Assembly",      scope: "stream", resp: "Supervises Technicians at the integration station; activates Solumina traveler steps; witnesses critical operations (torque, sealant); initiates OPRs; first escalation point for all parts shortages.",  auth: "Authorizes daily work sequence and tool checkout; can pause work for safety or quality cause. Cannot authorize part substitutions or drawing deviations." },
...
export function seedBoeing737(): { seeded: boolean; subStepsAdded: number } {
  ...
  repos.engagements.create(
    {
      ...
      client_org: "Spirit AeroSystems — 737 Fuselage Integration, Wichita Plant",
      notes: "OPR reduction engagement focused on Parts Missing & Omitted defects in the 737 forward fuselage integration stream. VP Supply Chain & Fabrication is the executive sponsor; Lean 6 Sigma Black Belt leading the internal team.",
    },
    E,
  );
  ...
  repos.value_streams.create(
    {
      engagement_id: E,
      name: "737 Integration — Parts Missing & Omitted (PM&O)",
      problem_statement:
        "The 737 fuselage integration line carries an average of 22 open PM&O OPRs per section, the majority traceable to BOM inaccuracies, phantom inventory, and late ECNs. Out-of-sequence installation to close OPRs adds an estimated 55 untracked labor hours per section, routinely extends station dwell beyond plan, and drives end-of-period overtime to meet Renton shipment commitments.",
      scope_level: "system",
      narrative:
  ...
  const data: DE[] = [
    ...
    { step: 1, name: "Engineering BOM",                  bind: "entry",  presence: "present", type: "BOM dataset",     src: "Spirit PDM",        key: true,  desc: "Released component requirements list that drives the kit list and SAP production order",               tbl: "SAP: MAST/STPO",       fld: "MATNR (component)",   ex: "P/N 65B32524-1" },
    { step: 1, name: "ECN Reconciliation Status",        bind: "entry",  presence: "partial", type: "status",          src: "Spirit PDM / ME",   key: true,  desc: "Confirmation that all active Engineering Change Notices are reflected in the issued traveler",        tbl: "(PDM — manual review)", fld: "ECN status",          ex: "3 of 4 ECNs incorporated", notes: "No automated check; ME manually reviews pending ECNs against the traveler at release — frequently done incompletely under schedule pressure" },
    { step: 1, name: "MSN (Manufacturing Serial Number)", bind: "entry", presence: "present", type: "string(8)",       src: "SAP PP",            key: true,  desc: "Unique serial number for this 737 fuselage section, linking all traveler and quality records",         tbl: "SAP: AFKO",            fld: "AUFNR / serialized",  ex: "737-1024" },
    { step: 1, name: "Solumina Traveler Release",        bind: "exit",   presence: "present", type: "traveler record", src: "Solumina (iBASEt)", key: false, desc: "Solumina traveler instance confirming this MSN's router is active and released to the floor" },
    ...
    { step: 4, name: "Solumina Kit Delivery Log",        bind: "exit",   presence: "partial", type: "timestamp",      src: "Solumina (iBASEt)", desc: "Solumina record of kit delivery to station with timestamp", notes: "Update is manual and frequently delayed; delivery time is unreliable for schedule tracking" },

    // Step 5 — Station Authorization
    { step: 5, name: "Tool Calibration Record",          bind: "entry",  presence: "partial", type: "certificate",    src: "Tool Crib system",  key: true,  desc: "Calibration certificate confirming all special tooling required by the traveler is within its calibration window", notes: "Calibration records exist in the tool crib system but are not linked to the Solumina traveler tooling requirement; Lead Mechanic verifies verbally" },
    ...
    { step: 5, name: "Traveler Step Activation",         bind: "exit",   presence: "present", type: "Solumina record", src: "Solumina (iBASEt)", desc: "Solumina confirmation that the Lead Mechanic has activated the traveler step for this MSN at this station" },

    // Step 6 — Structural Integration
    { step: 6, name: "Fastener Lot Traceability Record", bind: "action", presence: "partial", type: "record",         src: "Solumina / paper traveler", key: true, desc: "Lot number and quantity of each fastener type installed in the join zone, required for FAA airworthiness traceability", tbl: "Solumina: traveler step record", notes: "Lot numbers are manually recorded on the paper traveler under workload; barcode scan does not link to the fastener container. Wrong lot is occasionally recorded." },
    ...
    { step: 6, name: "Dimensional Sign-Off",             bind: "exit",   presence: "present", type: "sign-off",       src: "Solumina / paper traveler", key: true, desc: "Lead Mechanic witness signature confirming join dimensions are within drawing tolerance before sealant cure" },
    ...
    { step: 7, name: "Harness Routing Sign-Off",         bind: "action", presence: "partial", type: "sign-off",       src: "Solumina traveler", desc: "Solumina traveler step completion confirming harness routing is complete per drawing", notes: "Step completion is recorded but specific routing deviations or deferred adjustments are not captured" },

    // Step 8 — Post-Join Inspection
    { step: 8, name: "DI Traveler Inspection Sign-Off",  bind: "exit",   presence: "present", type: "sign-off",       src: "Solumina (iBASEt)", key: true,  desc: "Designated Inspector signature on each required inspection point in the Solumina traveler",              tbl: "Solumina: inspection records" },
    ...
    // Step 9 — Systems Test
    { step: 9, name: "Continuity Test Results",          bind: "exit",   presence: "partial", type: "test record",    src: "Solumina (iBASEt)", key: true,  desc: "Electrical continuity and insulation resistance measurements for each harness segment through the join zone", notes: "Pass/fail entered in Solumina; specific measured values not always recorded — traceability to root cause on failures is limited" },
    { step: 9, name: "Hydraulic Pressure Test Results",  bind: "exit",   presence: "present", type: "test record",    src: "Solumina traveler", desc: "Pressure test results for hydraulic lines in the integration zone" },

    // Step 10 — Section Buy-Off
    { step: 10, name: "Open OPR Status Report",          bind: "entry",  presence: "partial", type: "report",         src: "OPR tracking log (Excel)", key: true, desc: "List of all open OPRs against this MSN at time of buy-off review", notes: "Tracking log is maintained separately from SAP and Solumina; buy-off requires manual reconciliation of three systems. OPR status can lag actual closure by hours." },
    ...
    { step: 13, name: "Downstream Traveler Dependency Check", bind: "action", presence: "partial", type: "list", src: "Solumina (iBASEt — manual review)", desc: "Assessment of which downstream Solumina traveler steps are blocked by the missing part", notes: "ME manually traces dependencies in Solumina; no automated impact analysis tool exists" },

    // Step 14 — Supply Chain Expedite
    { step: 14, name: "Confirmed Supplier Delivery Date", bind: "exit",  presence: "partial", type: "date",           src: "Supplier portal / email",   key: true, desc: "Supplier-committed delivery date entered in OPR tracking log for production schedule replanning", notes: "Supplier date commitments are frequently revised; Spirit has limited real-time visibility into supplier manufacturing status. Average date-slip vs. first commitment is estimated at several days." },
    ...
    { step: 16, name: "Solumina Retroactive Traveler Update", bind: "exit", presence: "partial", type: "Solumina record", src: "Solumina (iBASEt)",      key: true, desc: "Retroactive completion of the affected Solumina traveler step to reflect OPR resolution", notes: "Retroactive updates are sometimes delayed or missed entirely, leaving the traveler in an incomplete state that re-flags at section buy-off review." },
  ];
  for (const d of data) {
  ...
  const metrics: MetricRow[] = [
    ...
    ["PM&O OPR Rate",                        "OPRs/100 install ops",  "quality",           3.2,  3.8,  0.5, false, "OPR tracking log vs. Solumina work content (notional ratio)"],
  ...
  const constraints: C[] = [
    ...
    },
    {
      ...
      desc: "Kit verification is performed by eye against a printed list with no barcode scan or system-enforced completeness check. There is no automated comparison between the Solumina kit list and the physical kit contents. A mismatch that is not caught here becomes a PM&O OPR on the integration floor, where the recovery cost is 2–5× the cost of catching it at kit inspection.",
    ...
    },
    {
      ...
      desc: "Spirit-designed and Spirit-sole-sourced parts — specific brackets, doublers, composite clips, and window-frame details — have no FAA-qualified alternate source. When these are the missing part, the section may sit at station for 10–21 days waiting for the supplier to manufacture and deliver a new run. Spirit has limited real-time visibility into supplier subcomponent availability, making expedite date-promises frequently unreliable.",
      kind: "risk", target: "step", step: 14, sev: "high", like: "medium",
    },
    {
      title: "OPR Tracking Disconnected from SAP and Solumina",
      desc: "The OPR backlog is maintained in a shared Excel spreadsheet that is not integrated with SAP QM or Solumina. This creates three separate data sources that must be manually reconciled at Section Buy-Off (Step 10), introduces transcription errors, and makes real-time OPR visibility impossible from any production dashboard or executive report. Status discrepancies between the Excel and SAP QN are common.",
  ...
  const cross: { from: ["step" | "persona", number]; to: ["step" | "persona", number]; type: "dependency" | "handoff" | "data_flow"; notes: string }[] = [
    ...
    { from: ["step", 16],  to: ["step", 10],  type: "dependency", notes: "OPR must be formally closed in SAP and Solumina before the section is eligible for buy-off." },
...
export function seedBoeing737SubSteps(): number {
  ...
  if (!repos.process_steps.get(FS6(1), { includeDeleted: true })) {
    ...
    const ftLot = repos.data_elements.create({ value_stream_id: V, name: "Fastener lot number (per row)", business_description: "Lot number of fasteners installed in each row of the join splice, required for airworthiness traceability", data_type: "string(20)", source_system: "Paper traveler / Solumina", table_or_view: "Solumina: traveler step record", field_name: "lot_ref (manual entry)", example_value: "LOT-2026-A44921", length: null });
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  const base = r.seeded ? "Spirit AeroSystems 737 engagement inserted" : "already present";
