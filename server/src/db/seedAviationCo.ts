...
export function seedAviationCo(): { seeded: boolean; subStepsAdded: number } {
  ...
  const data: DE[] = [
    ...
    { step: 1,  name: "ECN Reconciliation Status",             bind: "entry",  presence: "partial", type: "status",          src: "PDM / ME",                    key: true,  desc: "Confirmation that all active ECNs are reflected in the issued traveler",                                       tbl: "(PDM — manual review)",     fld: "ECN status",              ex: "3 of 4 ECNs incorporated", notes: "No automated check; ME manually reviews pending ECNs — frequently done incompletely under schedule pressure" },
  ...
  const artData: ArtDef[] = [
    { step: 1,  name: "Engineering BOM",                   artifact_type: "document",    form: "digital",  description: "Released component requirements list from PDM that drives the kit list and SAP production order." },
    { step: 2,  name: "SAP Component Requirements (RESB)", artifact_type: "document",    form: "digital",  description: "Component list generated from BOM for this production order; primary driver of the kit list." },
    { step: 2,  name: "Material Certification",            artifact_type: "document",    form: "digital",  description: "Certificate of Conformance or Material Test Report for traceable and life-limited items. Certs sometimes arrive separately from parts, creating a temporary hold." },
    { step: 2,  name: "Completed Kit Sheet",               artifact_type: "document",    form: "physical", description: "Signed confirmation of 100% kit completeness. No digital version; paper kit list with picker initials is the only record. Incomplete kits exit kitting undetected." },
    { step: 5,  name: "Tool Calibration Record",           artifact_type: "document",    form: "digital",  description: "Calibration certificate confirming special tooling is within its calibration window. Records exist in the tool crib system but are not linked to the MES traveler tooling requirement; Lead Mechanic verifies verbally." },
    { step: 10, name: "Open OPR Status Report",            artifact_type: "spreadsheet", form: "digital",  description: "List of all open OPRs against this MSN at time of buy-off review. Maintained in Excel, separate from SAP and MES; requires manual reconciliation of three systems." },
    { step: 10, name: "Section Conformance Document",      artifact_type: "document",    form: "digital",  description: "QE sign-off confirming all traveler steps are complete and all OPRs are closed or formally deferred." },
    { step: 11, name: "Paper NCR at Station",              artifact_type: "paper_file",  form: "physical", description: "Initial paper Non-Conformance Report written by Lead Mechanic at station at time of PM&O discovery. NCR forms not standardized across stations; incomplete fields create downstream problems for QE disposition." },
    { step: 14, name: "Premium Freight Authorization",     artifact_type: "document",    form: "digital",  description: "Written authorization for air freight or premium courier above the standard freight allowance. Requires VP approval above threshold; creates a decision bottleneck during high-OPR periods." },
  ];
...
export function seedAviationCoSubSteps(): number {
  ...
  if (!repos.process_steps.get(FS11(1), { includeDeleted: true })) {
    ...
    const ncrForm = repos.artifacts.create({ value_stream_id: V, name: "Paper NCR (at station)", artifact_type: "paper_file" as never, form: "physical" as never, description: "Initial paper Non-Conformance Report completed by Lead Mechanic at station at time of PM&O discovery. Forms not standardized; incomplete fields are common. Not retrievable electronically after departure from the station." });
    repos.step_artifacts.create({ step_id: FS11(3), artifact_id: ncrForm.id });
